...
class MessageApI {
    ...
    static func addMessage(uid:String,firstName:String,text: String, completion: @escaping (Bool) -> Void) {
        
        let refMessage = Firestore.firestore().collection("Message")
        refMessage.document(uid).setData(Message.createMessage(firstName: firstName, text: text))
